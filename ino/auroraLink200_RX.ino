/* * PROJECT:  Aurora-Link v200-RX (Final Hardened)
 * STRATEGY: Idle-Low / Hysteresis / Hamming / Signal Reporting
 * FILE:     auroraLink200_RX.ino
 * COMPILED: Mar 04, 2026 | 08:15:00
 */

uint32_t userBaud = 9600; 
const int SENSOR_PIN = A0;
const int REPORT_PIN = 3; // Ground this pin to REPORT signal values (Debug)
...
bool getSample() {
  ...
  // SIGNAL REPORTING: If Pin 3 is grounded, print raw levels
  if (digitalRead(REPORT_PIN) == LOW) {
    Serial.print(F("SIGNAL: ")); Serial.print(val);
    Serial.print(F(" | BEAM: ")); Serial.println(beamState ? "LIT" : "DARK");
    delay(40); // Sane reporting speed
  }
...
void setup() {
  ...
  Serial.println(F("========================================"));
  Serial.println(F("PROJECT: AURORA-LINK v200-RX"));
  ...
  Serial.println(F("REPORT:  Ground Pin 3 to view Signal Value"));
  Serial.println(F("========================================"));
}

void loop() {
  // 1. Wait for Wake-Up Burst (Link Activation)
  if (!linkActive) {
    if (getSample()) { 
      linkActive = true;
    ...
    // 3. EOB CHECK: Timeout if no light for 5 bit-periods
